...
const assessLotteryCompIndex = 3
...
func before_test():
	criterion = manager.criteria[assessLotteryCompIndex]
	criterion.setup()
...
func test_answer_p_question():
	...
	assert_float(criterion.question.get_right().win_value).is_equal(10.0)
	assert_float(criterion.question.get_right().win_probability).is_equal(0.5625)
	assert_float(criterion.question.get_right().loss_value).is_equal(0.0)
...
func test_answer_q_question():
	...
	assert_float(criterion.question.get_right().win_value).is_equal(10.0)
	assert_float(criterion.question.get_right().win_probability).is_equal(0.1875)
	assert_float(criterion.question.get_right().loss_value).is_equal(0.0)
