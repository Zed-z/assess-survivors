...
const assessVariableProbability = 2
...
func before_test():
	criterion = manager.criteria[assessVariableProbability]
	criterion.setup()
...
func test_answer_p():
	...
	assert_float(criterion.point_list[1].x).is_equal_approx(10.0/6, 0.01)
	assert_float(criterion.point_list[1].y).is_equal_approx(1.0/3, 0.01)


func test_answer_p_question():
	criterion.step(AssessCriterion.Answer.p)

	assert_float(criterion.question.get_left().win_value).is_equal_approx(10.0/6, 0.01)
	assert_float(criterion.question.get_left().win_probability).is_equal_approx(1.0, 0.01)
	assert_float(criterion.question.get_left().loss_value).is_equal_approx(-1.0, 0.01)

	assert_float(criterion.question.get_right().win_value).is_equal_approx(10.0, 0.01)
	assert_float(criterion.question.get_right().win_probability).is_equal_approx(1.0/3, 0.01)
	assert_float(criterion.question.get_right().loss_value).is_equal_approx(0.0, 0.01)


func test_answer_q():
	criterion.step(AssessCriterion.Answer.q)
	assert_float(criterion.point_list[1].x).is_equal_approx(20.0/3, 0.01)
	assert_float(criterion.point_list[1].y).is_equal_approx(1.0/3, 0.01)
...
func test_answer_q_question():
	...
	assert_float(criterion.question.get_left().win_value).is_equal_approx(20.0/3, 0.01)
	assert_float(criterion.question.get_left().win_probability).is_equal_approx(1.0, 0.01)
	assert_float(criterion.question.get_left().loss_value).is_equal_approx(-1.0, 0.01)
...
func test_answer_i():
	...
	assert_almost_eq_vector2_array(criterion.point_list, [Vector2(0, 0),Vector2(10/3.0,1.0/3), Vector2(20.0/3, 2.0/3),Vector2(10, 1)], 0.01)
...
func test_answer_i_question():
	...
	assert_float(criterion.question.get_right().win_value).is_equal_approx(10.0, 0.01)
	assert_float(criterion.question.get_right().win_probability).is_equal_approx(2.0/3, 0.01)
	assert_float(criterion.question.get_right().loss_value).is_equal_approx(0.0, 0.01)
